...
class Alphamodel():
    ...
    def calc_single_factor_IC_value(self, factor_path, prices_frame, start_year, end_year):
        '''
            IC的具体值：
    ...
    def calc_all_factors_IC_value(self, factors_path, prices_path, start_year, end_year):
        factors_dirs = os.listdir(factors_path)

        prices_frame = dataset.load_period_prices(prices_path, start_year, end_year, regenerate=False)
        ...
        for factors_dir in factors_dirs:
            if factors_dir.split('_')[0] == 'barra':
                ...
                factors = os.listdir(os.path.join(factors_path, factors_dir, 'Factor'))
                for factor in factors:
                    factor_data = self.calc_single_factor_IC_value(os.path.join(factors_path, factors_dir, 'Factor', factor), prices_frame, start_year, end_year)
